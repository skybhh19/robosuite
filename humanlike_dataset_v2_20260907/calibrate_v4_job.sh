...
if [ "$MODE" = prepare ]; then
  "$PY" "$ROOT/collect.py" prepare --kind "$KIND" --human-version human_v4 \
    --root "$ROOT/calibration_v4/$KIND" --seed 202609075 --pairs 20
else
  "$PY" "$ROOT/collect.py" run --kind "$KIND" --human-version human_v4 \
    --root "$ROOT/calibration_v4/$KIND" --index "${SLURM_ARRAY_TASK_ID:-0}" --shards 20
fi
