...
def trial(a,m,e,version,regime):
 ...
 try:
  if a.kind=='threading':
   ...
   from robosuite.scripts.collect_tool_hang_wrench_joint import GeometricJointPolicy,ROBUST_JOINT_OPTIONS,THREADING_STYLE_JOINT_OPTIONS,TOOLHANG_VLA_V4_OPTIONS,TOOLHANG_VLA_V5_OPTIONS,TOOLHANG_VLA_V6_OPTIONS,VideoRecorder,collection_acceptance
   ...
   if version == 'toolhang_vla_v2':
    # Wider fixture randomization produces larger residual errors near the
    # hook. Keep the same policy in both regimes, but allow more measured
   ...
   preset=(TOOLHANG_VLA_V6_OPTIONS if version == 'toolhang_vla_v6' else TOOLHANG_VLA_V5_OPTIONS if version == 'toolhang_vla_v5' else TOOLHANG_VLA_V4_OPTIONS if version == 'toolhang_vla_v4' else THREADING_STYLE_JOINT_OPTIONS if version == 'toolhang_vla_v3' else ROBUST_JOINT_OPTIONS)
