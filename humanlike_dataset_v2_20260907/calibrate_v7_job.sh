...
if [ "$MODE" = prepare ]; then
  "$PY" "$ROOT/collect.py" prepare --kind threading --human-version human_v7 \
    --root "$ROOT/calibration_v7/threading" --seed 202609078 --pairs 60
else
  "$PY" "$ROOT/collect.py" run --kind threading --human-version human_v7 \
    --root "$ROOT/calibration_v7/threading" --index "${SLURM_ARRAY_TASK_ID:-0}" --shards 60
fi
